...
# (bash arrays can't be exported directly)
...
for s in meta:
    sid  = s["sample_id"]
    r1   = f"{fq_dir}/{sid}_R1.fastq.gz"
    r2   = f"{fq_dir}/{sid}_R2.fastq.gz"
    pfx  = f"{sal_d}/{sid}"
    done = f"{pfx}/quant.sf"
    ...
    needed = [r1, r2] if paired else [r1]
    for fq in needed:
        if not os.path.exists(fq):
            print(f"  ERROR: FASTQ not found: {fq}")
            print(f"         Run:  python3 scripts/organize_fastqs.py --help")
            sys.exit(1)
